...
def compare_folders(
    ...
    def print_files_missing_search(
        def_files,
        def_path,
        def_info_text,
    ):
        ...
        for idx, (file, path) in enumerate(
            ...
            found = search_file(
                os.path.basename(file), search_path=def_path
            )
            if found:
                print(
                    f"                                -> but found in target folder: {found}"
                )
        if def_files.items():
            print()
        return count_files
    ...
    # )
    count_files_missing_in_target = print_files_missing_search(
        files_missing_target,
        def_folder_target,
        "ERROR:     File missing in target",
    )
